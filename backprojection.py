...
for z in range(len(positions)):
    ...
    for j in range(grid_resolution[0]):
        for k in range(grid_resolution[1]):
            ...
            distance = np.sqrt(
                (positions[z][0] - pixel_coords_meters[0]) ** 2
                + (positions[z][1] - pixel_coords_meters[1]) ** 2
                + positions[z][2] ** 2
            )
            index = np.argmin(
                np.abs(range_bins - distance)
            )  # Find index of the closest range bin
            added_intensities[k][j] += db_set[z][index]  # Adds the amplitude at the pixel coordinates for each frame

added_intensities /= 200 # Normalize 
plt.imshow(
    added_intensities,
